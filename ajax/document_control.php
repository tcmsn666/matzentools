...
try{
 if($action==='save_check'){
  ...
  $id=postInt('id'); $stmt=db()->prepare('SELECT oqd.document_id, cv.id AS current_version_id FROM object_qm_documents oqd JOIN qm_document_versions cv ON cv.document_id=oqd.document_id AND cv.is_current=1 WHERE oqd.id=?'); $stmt->execute([$id]); $row=$stmt->fetch(); if(!$row) jsonResponse(['success'=>false,'message'=>'Zuordnung nicht gefunden.'],404);
  db()->prepare('UPDATE object_qm_documents SET document_version_id=?, last_checked_at=CURDATE(), updated_by=?, updated_at=NOW() WHERE id=?')->execute([(int)$row['current_version_id'],(int)currentUser()['id'],$id]);
 }
