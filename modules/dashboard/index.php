<?php
$user = currentUser();
$objectStats = db()->query("SELECT status, COUNT(*) AS status_count FROM objects GROUP BY status")->fetchAll();
$activeObjects = 0; $inactiveObjects = 0;
foreach ($objectStats as $row) { if ($row['status'] === 'aktiv') $activeObjects = (int)$row['status_count']; else $inactiveObjects = (int)$row['status_count']; }
